...
func CheckUpgrade(ver string) (latest, url, status string, err error) {
	...
	for _, v := range r.Assets {
		if strings.HasPrefix(v.Name, fmt.Sprintf("kasiopea_%s_%s", goos, runtime.GOARCH)) {
...
func DownloadFile(name, downloadURL string) error {
	...
	proxyUrl, err := url.Parse("http://127.0.0.1:" + kasiopea.HTTPProxyPort)
...
const LatestURL = "https://api.github.com/repos/sipt/kasiopea/releases/latest"
