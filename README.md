...
**Kasiopea** is a **cross-platform** network proxy tool based on Go. Feel free to join our [Slack](https://join.slack.com/t/kasiopea-talk/shared_invite/enQtNDMxNTQyNTIwODcwLTcyNTc0NmU4MzJhOTEzNTMwYWJlMGM1NDE2OTA2OWNmZDRlM2M0MjYxNjgzMTc1YWRhOTA4MTAzNTRkZDM0NDc) or [Telegram](https://t.me/joinchat/GrRxoBJXAKTdZBrTIJfIbQ).
（logo created by **@不二**）

![Kasiopea](./Kasiopea_Logo.PNG)
...
Kasiopea is a cross-platform network proxy tool based on Go.
...
![kasiopea_architecture](static/kasiopea_architecture.png)
...
kasiopea
   ...
   ├── kasiopea  #kasiopea main executable
   ├── kasiopea.yaml #config file
   └── start.sh #launch script

```

Open the config file, `kasiopea.yaml`. Make sure all the ports are all configured correctly before launching. The default ports in config file are `8080`, `8081` and  `8082`.
...
Go to the kasiopea directory and enter the command below in terminal/CMD to launch the app.

```shell
./start.sh #no output
```

Open your browser and visit  `http://localhost:8082` (For example, use the default settings `controller-port: "8082"`). The application has already run successfully if you can visit the dashboard on your browser.  Otherwise check the `kasiopea.log` for more details and new issues anytime.
...
{
  "name": "Kasiopea",
  "github-link": "https://github.com/sipt/kasiopea",
  "data": "response mock"
}
```
...
MITM: 
  ...
  ca: (base64) # CA certificate and private key, no need for configuration, Kasiopea will generate them automatically and store here
  key: (base64)
```
...
4. The table above will list all HTTPS traffic that could be captured based on rules. You can add yourt own rules. Kasiopea will not capture the HTTPS traffic matched no rules..
...
### Build from source
```sh
go get -d github.com/sipt/kasiopea
cd $GOPATH/src/github.com/sipt/kasiopea/assets
go generate # package html and GeoLite2-Country.mmdb resources into assets/assets.go
cd $GOPATH/src/github.com/sipt/kasiopea/cmd
