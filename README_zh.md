...
**Kasiopea**是一个基于Go开发的**全平台**网络代理工具。欢迎加入讨论 [Slack](https://join.slack.com/t/kasiopea-talk/shared_invite/enQtNDMxNTQyNTIwODcwLTcyNTc0NmU4MzJhOTEzNTMwYWJlMGM1NDE2OTA2OWNmZDRlM2M0MjYxNjgzMTc1YWRhOTA4MTAzNTRkZDM0NDc) or [Telegram](https://t.me/joinchat/GrRxoBJXAKTdZBrTIJfIbQ)。
（logo**@不二**）
...
kasiopea
   ├── RespFiles/ #mock文件存放
   ├── kasiopea  #kasiopea主程序
   ├── kasiopea.yaml #配置文件
   └── start.sh #启动脚本

```

打开配置文件：`kasiopea.yaml`，启动前要注意的是端口号冲突，配置文件中预设的是：`8080`,`8081`,`8082`
...
此时不会有任何输出，此时在浏览器中打开`http://localhost:8082`（以`controller-port: "8082"`为例），如果能打开控制台页面就说明启动成功，如果打开失败可以查看`kasiopea.log`查看原因，如果排查不出原因可以去提`Issues`。
...
双击打开`startup.bat`，此时不会有任何输出，此时在浏览器中打开`http://localhost:8082`（以`controller-port: "8082"`为例），如果能打开控制台页面就说明启动成功，如果打开失败可以查看`kasiopea.log`查看原因，如果排查不出原因可以去提`Issues`。
...
1. GLOBAL分组由Kasiopea创建，在“Remote Mode”时会使用该分组。
...
7. Reload：重新加载配置文件；Shutdown：关闭Kasiopea。
