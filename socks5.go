...
import (
	...
	"github.com/sipt/kasiopea/pool"
	"github.com/sipt/kasiopea/proxy"
	"github.com/sipt/kasiopea/util"
)
...
func SocksHandle(co net.Conn) {
	log.Logger.Debug("[SOCKS] start kasiopea.IConn wrap net.Conn")
	conn, err := connect.NewDefaultConn(co, connect.TCP)
	if err != nil {
		log.Logger.Errorf("kasiopea.IConn wrap net.Conn failed: %v", err)
		return
	}
	log.Logger.Debugf("[SOCKS] [ID:%d] kasiopea.IConn wrap net.Conn success ", conn.GetID())
