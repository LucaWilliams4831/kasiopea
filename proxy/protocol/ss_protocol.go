...
func AddressEncoding(req sproxy.IRequest) ([]byte, error) {
	var atyp uint8 = kasiopea.AddrTypeDomain
	ip := net.ParseIP(req.Domain())
	addr := []byte(req.Domain())
	if ip != nil {
		if len(ip) == net.IPv4len {
			atyp = kasiopea.AddrTypeIPv4
		} else {
			atyp = kasiopea.AddrTypeIPv6
		}
		addr = []byte(ip)
	}
	if len(addr) == 0 {
	...
	switch atyp {
	case kasiopea.AddrTypeIPv4, kasiopea.AddrTypeIPv6:
		buffer.WriteByte(atyp)
		buffer.Write(addr)
		buffer.Write(portBytes)
	case kasiopea.AddrTypeDomain:
		buffer.WriteByte(atyp)
