...
import (
	...
	rule2 "github.com/sipt/kasiopea/rule"
...
func HandleHTTP(co net.Conn) {
	...
	if err != nil {
		log.Logger.Errorf("[HTTP] kasiopea.IConn wrap net.Conn failed: %v", err)
		return
	}
	log.Logger.Debugf("[HTTP] [ID:%d] kasiopea.IConn wrap net.Conn success", conn.GetID())
